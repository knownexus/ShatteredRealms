...
public sealed class UpdateOwnProfileCommandHandler : IRequestHandler<UpdateOwnProfileCommand, Result<UserDto>>
{
    private readonly IUserService _userService;
    private readonly IPermissionService _permissionService;
    private readonly ApplicationDbContext _context;

    public UpdateOwnProfileCommandHandler(
    ...
    public async Task<Result<UserDto>> Handle(UpdateOwnProfileCommand request, CancellationToken cancellationToken)
    {
        ...
        var userResult = await _userService.UpdateUserAsync(request.UserId, new UpdateUserRequest
        {
            Email     = request.Email,
